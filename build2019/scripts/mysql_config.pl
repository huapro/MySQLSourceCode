...
#        handle -lz  and other strings inserted that might contain
...
my $ldata   = '/Users/yanglihua/Downloads/percona-server-5.6.43/work/data';
my $execdir = '/Users/yanglihua/Downloads/percona-server-5.6.43/work/bin';
my $bindir  = '/Users/yanglihua/Downloads/percona-server-5.6.43/work/bin';

# ----------------------------------------------------------------------
# If installed, search for the compiled in directory first (might be "lib64")
# ----------------------------------------------------------------------

my $pkglibdir = fix_path('/Users/yanglihua/Downloads/percona-server-5.6.43/work/lib',"libmysql/relwithdebinfo",
                         "libmysql/release","libmysql/debug","lib/mysql","lib");

my $pkgincludedir = fix_path('/Users/yanglihua/Downloads/percona-server-5.6.43/work/include', "include/mysql", "include");
...
$flags->{libs} =
  [@ldflags,@lib_opts,'','-lz ','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'','-lz ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','','-lz ','',''];

$flags->{include} = ["-I$pkgincludedir"];
$flags->{cflags}  = [@{$flags->{include}},split(" ",'-O3 -g -fno-omit-frame-pointer -fno-strict-aliasing -DDBUG_OFF')];
$flags->{cxxflags}= [@{$flags->{include}},split(" ",'-O3 -g -fno-omit-frame-pointer -fno-strict-aliasing -DDBUG_OFF')];
